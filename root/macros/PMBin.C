...
TFile datafile("histograms/FullCompton.root");
...
void GetSim( Int_t ebin, Int_t tbin, Double_t open_cut)
{
	...
	name = Form( "histograms/MonteCarlo/compton/compton_p_%d.root",
			tch[ebin].eg);
	comp = new TFile( name);

	name = Form( "histograms/MonteCarlo/compton/pi0_p_%d.root", tch[ebin].eg);
	back = new TFile( name);
