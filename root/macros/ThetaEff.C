...
void EffRange( UInt_t start, UInt_t end, TString savedir = "pi0",
		TString encl = "LH2_10cm", TString proc = "pi0_p", UInt_t rebin = 20)
{
	UInt_t i;

	for ( i = start; i <= end; ThetaEff( i++, savedir, encl, proc, kFALSE,
				rebin));
}

void ThetaEff( UInt_t chan = 300, TString savedir = "pi0",
		...
		UInt_t rebin = 20)
{
	...
	name = "evgen/tageng/" + base + "_in.root";
	evgen = new TFile( name);

	name = "histograms/MonteCarlo/" + savedir + "/" + encl + "/" + base +
		".root";
...
//	name = "THR_ThetaCMP";
	...
	name = Form( "xs/eff/det_eff_new/Eff_%s_chan_%d.out", (const char*) proc,
			chan);
