...
void TGG2DCut( Int_t bin = 0, TString tgt = "p")
{
	...
	name = Form( "histograms/MCH_pi0_%s_%d.root", (const char*) tgt, eg[bin]);
	sim = new TFile( name);
	...
	name = Form( "evgen/pi0_%s_%d_in.root", (const char*) tgt, eg[bin]);
	ev = new TFile( name);
	name = "h5";
	...
	for ( tbin = 0; tbin < 9; tbin++)
	{
		...
		if ( i == 1)
		{
			name = Form( "E_{#gamma} = %3d MeV, Target = %s",
					eg[bin], (const char*) tgt, th_lo, th_hi);
		}
		else name = "";
		h2->SetTitle( name);
	...
	name = Form( "plots/pdf/TGG_v_Mmiss_%s_%d.pdf", (const char*) tgt, eg[bin]);
	c1->Print( name);
}
